 ...
 */
contract ERC20OA is Context, IERC20, IERC20Metadata {
    using Counters for Counters.Counter;
    Counters.Counter private _holdersCounter;
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_, address _adminAddress, address _marketingAddress, address _developmentAddress, address _helpingAddress, address _rewardsAddress) {
        _name = name_;
        _symbol = symbol_;
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        _holders[_holdersCounter.current()] = account;
        _holdersCounter.increment();
    ...
    function isHolder(address _address) private view returns (bool) {
        uint256 totalHolders = _holdersCounter.current();
        for (uint i = 0; i < totalHolders; i++) {
            if (_holders[i] == _address) {
                return true;
            }
        }
    ...
    function getHoldersQuantity () private view returns (uint256) {
        uint256 totalHolders = _holdersCounter.current();
        uint256 counterHolders = 0;
        for (uint i = 0; i < totalHolders; i++) {
            if (_balances[_holders[i]] > 0) {
                counterHolders++;
            }
