...
contract NFTMarketOA is ReentrancyGuard {
    ...
    constructor(uint256 _listingPrice, address address_backup) payable {
        ...
        for (uint256 item = 0; item < oldData.length; item++) {
            ...
            marketItems[itemId] = MarketItem(
                ...
                payable(oldData[item].owner), oldData[item].price, oldData[item].sold,
                oldData[item].onAuction, oldData[item].onSale, oldData[item].endTime,
                oldData[item].highestBidder, oldData[item].highestBid, oldData[item].currency,
    ...
    function createMarketItem(address nftContract, uint256 tokenId, bool isActive) public {
        ...
        uint256 itemId = _itemIds.current();
        marketItems[itemId] = MarketItem(
            itemId, nftContract, tokenId, payable(msg.sender), 0, false, false, false, 0, address(0), 0, address(0), isActive
        );

        emit MarketItemCreated(
            itemId, nftContract, tokenId, msg.sender, 0, false, false, false, 0, address(0), 0
        );
    }
    ...
    function createMarketSale(address nftContract, uint256 itemId) public payable nonReentrant {
        ...
        require(
            marketItems[itemId].onSale,
            "This Item is not on sale."
        );

        require(marketItems[itemId].owner != msg.sender, "You are the owner of this nft");

        require(marketItems[itemId].currency != address(0) || msg.value == marketItems[itemId].price, "The value sent must be equals to nft's price");
    ...
    function setListingPrice (uint percent) public {
    ...
    function activateSale(uint256 itemId, uint256 price, address currency) public {
        ...
        IERC721(marketItems[itemId].nftContract).transferFrom(marketItems[itemId].owner, address(this), marketItems[itemId].tokenId);
    ...
    function deactivateSale(uint256 itemId) public {
        require(msg.sender == marketItems[itemId].owner, "You are not owner of this nft");
        IERC721(marketItems[itemId].nftContract).transferFrom(address(this), marketItems[itemId].owner, marketItems[itemId].tokenId);
        marketItems[itemId].onSale = false;
        marketItems[itemId].onAuction = false;
    }
    ...
    function bid(uint256 itemId, uint256 bidAmount) public {
        ...
        require((bidAmount > marketItems[itemId].highestBid) || (bidAmount == marketItems[itemId].price && marketItems[itemId].highestBid == 0), "There is already a higher or equal bid");
    ...
    function auctionEnd(uint256 itemId) public {
        ...
        if (marketItems[itemId].owner == owner) {
            ...
            require(erc20.transfer(marketItems[itemId].owner, (marketItems[itemId].highestBid - (marketItems[itemId].highestBid * listingPrice / 100))), "Transfer to owner failed");
    ...
    function transferNFT(address nftContract, address to, uint256 nftId) public {
    ...
    function setActiveItem(uint256 itemId, bool isActive) public{
        require(msg.sender == owner || msg.sender == marketItems[itemId].owner, "You are not allowed to modify this element");
