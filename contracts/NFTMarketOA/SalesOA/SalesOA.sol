...
contract SalesOA is ReentrancyGuard {
    ...
    function createMarketSale(uint256 itemId, address buyer) public payable nonReentrant {
    ...
    /* Put on sale */
    function activateSale(uint256 itemId, uint256 price, address currency, address seller) public {
        IStorageOA iStorage = IStorageOA(address_storage);
        IStorageOA.StorageItem memory item = iStorage.getItem(itemId);
    ...
    /* Remove from sale */
    function deactivateSale(uint256 itemId, address seller) public {
        IStorageOA iStorage = IStorageOA(address_storage);
        IStorageOA.StorageItem memory item = iStorage.getItem(itemId);
        require(seller == item.owner, "You are not owner of this nft");
